          ...
          <tbody>
            {tours.map((tour, index) => (
              <tr key={tour.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-2 border border-gray-300 text-right text-green-600">{tour.price}</td>
                <td className="px-4 py-2 border border-gray-300 text-center">{tour.available} chỗ</td>
                <td className="px-4 py-2 border border-gray-300 text-center">
                  <button onClick={() => Delete(tour.id)}>Xóa</button>
